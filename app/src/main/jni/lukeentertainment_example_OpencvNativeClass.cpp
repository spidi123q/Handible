...
JNIEXPORT jint JNICALL Java_lukeentertainment_example_OpencvNativeClass_processImage
    (JNIEnv *env, jclass,jlong addrRgba){
     ...
     cv::Mat erosionMat;
     cv::Mat matThreshCopy;
     int width=matTestingNumbers.cols;
    int height=matTestingNumbers.rows;
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG","\n R : %d, C : %d",r,r);
    cv::GaussianBlur(matGrayscale,matBlurred,cv::Size(r,r),0);
    ...
    Mat lineStructEle= Mat::ones(3,3, CV_8UC1);
    cv::erode(matThresh,matThresh,lineStructEle);
...
JNIEXPORT jint JNICALL Java_lukeentertainment_example_OpencvNativeClass_detectWords
 ...
 JNIEXPORT jint JNICALL Java_lukeentertainment_example_OpencvNativeClass_encryptTrain
  ...
  cv::Mat& mRgb=*(cv::Mat*)addrRgba;
     ...
     const char* strChars = env->GetStringUTFChars(path, (jboolean *)0);
     std::string Result(strChars, len);
     char classPath[100],imagePath[100],namePath[100];

     strcpy(classPath,strChars);
     strcpy(imagePath,strChars);
     strcpy(namePath,strChars);

     env->ReleaseStringUTFChars(path, strChars);
     strcat(classPath,"encryptClass.xml");
     strcat(imagePath,"encryptKeypoints.xml");
     __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG"," %s\n",classPath);
     ...
     cv::findContours(imgThreshCopy,ptContours,v4iHierarchy,cv::RETR_EXTERNAL,cv::CHAIN_APPROX_SIMPLE);
      ...
      std::sort(validContoursWithData.begin(),validContoursWithData.end(), ContourWithData::sortByBoundingRectYPosition);
      int intChar='0';
 ...
 __android_log_print(ANDROID_LOG_DEBUG, "LOG_TAG","\n %c",t[0]);
            ...
            cv::Mat matROIResized;
            cv::resize(matROI, matROIResized, cv::Size(RESIZED_IMAGE_WIDTH, RESIZED_IMAGE_HEIGHT));     // resize image, this will be more consistent for recognition and storage
             matClassificationInts.push_back(int(intChar));
           ...
           matClassificationInts.convertTo(matClassificationFloats,CV_32FC1);
           cv::FileStorage fsClassifications(classPath, cv::FileStorage::WRITE);           // open the classifications file
           fsClassifications << "encryptClass" <<matClassificationInts;        // write classifications into classifications section of classifications file
           fsClassifications.release();                                            // close the classifications file

           cv::FileStorage fsTrainingImages(imagePath, cv::FileStorage::WRITE);         // open the training images file

           fsTrainingImages << "encryptKeypoints" << matTrainingImages;         // write training images into images section of images file
       fsTrainingImages.release();
       mRgb=imgTrainingNumbers;
       return (jint)intChar;
   }
...
cv::Mat& mRgb=*(cv::Mat*)addrRgba;
    ...
    fsClassifications["encryptClass"] >> matClassificationFloats;      // read classifications section into Mat classifications variable
    fsClassifications.release();                                        // close the classifications file
    ...
    fsTrainingImages["encryptKeypoints"] >> matTrainingImages;           // read images section into Mat training images variable
    fsTrainingImages.release();
                                                // close the traning images file
    cv::KNearest kNearest=cv::KNearest();
    kNearest.train(matTrainingImages,matClassificationFloats);
    ...
    cv::cvtColor(testingNumbers,gray, CV_RGB2GRAY);
